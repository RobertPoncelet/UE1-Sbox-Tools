...
objpaths = glob.glob(os.path.join(objroot, "**", "*.obj"), recursive=True)
...
for mdl in mdlpaths:
    ...
    # Our simple script doesn't currently put materials in the OBJ
    '''mats = []
    with open(obj) as objfile:
        for line in objfile:
            words = line.split()
            if len(words) == 2 and words[0] == "usemtl":
                mats.append(words[1])
    print("Materials: " + str(mats))'''
    mats = ["default"]
